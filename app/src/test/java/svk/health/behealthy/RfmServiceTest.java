...
import svk.health.behealthy.service.RfmService;
...
public class RfmServiceTest {
    ...
    @Test
    public void testCalculateRfm_man() {
        ...
        RfmCategory response = this.rfmService.calculateRfm("180", "90", "32", "man");

        Assert.assertEquals(Double.valueOf(24.0), Double.valueOf(response.getCalculatedRfm()));
        Assert.assertEquals("Optimal weight", response.getCategory());
    }

    @Test
    public void testCalculateRfm_woman() {
        JsonProperty jsonProperty = new JsonProperty(generateAgeWithBmis(), Collections.emptyList(), Collections.emptyList(), Collections.emptyList(), Collections.emptyList());

        Mockito.when(jsonService.processFile(R.raw.agewithbmis)).thenReturn(jsonProperty);

        RfmCategory response = this.rfmService.calculateRfm("180", "90", "32", "woman");

        Assert.assertEquals(Double.valueOf(36.0), Double.valueOf(response.getCalculatedRfm()));
        Assert.assertEquals("Obesity", response.getCategory());
    }
